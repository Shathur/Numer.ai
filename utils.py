...
def read_csv(file_path, rows_num=0, skip_rows_num=0, load_val=False):
    ...
    if load_val:
        ...
        numerai_datasets_path = os.path.join(
            os.path.dirname(__file__), "numerai_datasets"
        )
        read_rows_skip = pd.read_csv(
            os.path.join(numerai_datasets_path, "validation_indexes.csv")
        )
        ...
        df = pd.read_csv(
            os.path.join(numerai_datasets_path, "numerai_tournament_data.csv"),
            skiprows=skip_rows,
            dtype=dtypes,
        )
        # df[df['era'] == 'era852']
        ...
        if rows_num == 0:
            ...
            df = pd.read_csv(
                file_path,
                skiprows=[i for i in range(1, skip_rows_num)],
                dtype=dtypes,
...
def corr_score(df, pred_name, target_name="target", group_name="era"):
    ...
    correlations = df.groupby(group_name).apply(
        lambda x: score(x, pred_name, target_name)
    )
    return correlations
...
def feature_exposures_parallel(df, pred_name):
    with ThreadPoolExecutor() as executor:
        ...
        for feat in feature_names:
            _futures.append(executor.submit(spearmanr, a=df[pred_name], b=df[feat]))
...
def get_feature_neutral_mean(df, pred_name, target_name):
    ...
    df.loc[:, "neutral_sub"] = neutralize(df, [pred_name], feature_cols)[pred_name]
    scores = (
        df.groupby("era")
        .apply(lambda x: spearman(x["neutral_sub"], x[target_name]))
        .mean()
    )
...
def plot_feature_exposures(df, pred_name):
    ...
    ax.legend(
        title="Max_feature_exposure : {}\n"
        "Mean feature exposure : {}".format(
            max_feature_exposure(df, pred_name), feature_exposure(df, pred_name)
        )
    )
...
def plot_corrs_per_era(
    df, pred_name="prediction", target_name="target", group_name="era"
):
...
def plot_feature_neutralization(
    tour_df,
    neut_percent,
    target_name="target",
    ...
    ax.legend(
        title="Max_feature_exposure : {}\n"
        "Mean feature exposure : {}".format(
            max_feature_exposure(validation_data, pred_name),
    ...
    validation_data[pred_name + "_neutralized"] = neutralize_short(
        validation_data, prediction_name=pred_name, proportion=neut_percent
    )
    ...
    val_corrs_neut = corr_score(
        validation_data, pred_name + "_neutralized", target_name, group_name
    )
    ...
    if show_metrics:
        metrics = print_metrics(
            tour_df=validation_data, pred_name=pred_name, long_metrics=False
        )
        if scores_on_val2:
            metrics = print_metrics(
                tour_df=validation_data,
                pred_name=pred_name,
                long_metrics=False,
                scores_on_val2=True,
            )
    ...
    val_corrs_neut_per_era = corr_score(
        validation_data, PRED_NAME_NEUT_PER_ERA, target_name, group_name
    )
    ...
    plt.figure()
    ax1 = sns.barplot(
        x=val_corrs_neut_per_era.index.str.slice(3), y=val_corrs_neut_per_era
    )
...
def print_metrics(
    ...
    if val_df is not None:
        ...
        print(
            f"On oof the correlation has mean {oof_correlations.mean()} and "
    ...
    if tour_df is not None:
        if scores_on_val2 == False:
            ...
            # Check the per-era correlations on the validation set (out of sample)
            validation_data = tour_df[tour_df.data_type == "validation"]
            validation_correlations = corr_score(
                validation_data, pred_name, target_name, group_name
            )
            print(
            ...
            print(
                f"On validation_2 the correlation has mean {validation_correlations.mean()} and "
                f"std {validation_correlations.std(ddof=0)}"
            )
    ...
    if long_metrics == True:
        ...
        if val_df is not None:
            ...
            feature_exposures = val_df[feature_names].apply(
                lambda d: spearman(val_df[pred_name], d), axis=0
            )
        ...
        feature_exposures = validation_data[feature_names].apply(
            lambda d: spearman(validation_data[pred_name], d), axis=0
        )
    ...
    if no_fn:
        ...
        if neut_type == "short":
            df[pred_name + "_neutralized"] = neutralize_short(
                df, prediction_name=pred_name, proportion=proportion
            )
