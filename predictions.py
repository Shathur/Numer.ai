...
def get_predictions_per_era_joblib(df, preds_cache_file=None, num_models=1, prefix=None,
    ...
    for cv_num in tqdm(range(num_models)):
        if cv_num > int(list(cache)[0]):  # check if the model predictions have been saved
            ...
            cache = {cv_num: predictions_final.squeeze()}
...
"""
the code below works like a charm but works under the assumption that the size of the tournament
data from Numer.ai stays the same. This assumption is not correct since eraX becomes era980 for example
and turns from live to backtesting. The code can give accurate and fast results when calculating the 
cache file for the first time. It cannot be used though after the first week.
"""


def get_predictions_per_era_joblib_initial(df, preds_cache_file=None, num_models=1, prefix=None,
